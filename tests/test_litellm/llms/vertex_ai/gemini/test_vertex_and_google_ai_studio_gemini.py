...
def test_accumulated_json_uses_list_buffer():
    """accumulated_json_chunks must be a list, not a str; appending must not copy the whole buffer."""
    from litellm.llms.vertex_ai.gemini.vertex_and_google_ai_studio_gemini import (
        ModelResponseIterator,
    )
    ...
    assert isinstance(iterator.accumulated_json_chunks, list)
    assert iterator.accumulated_json_chunks == []


def test_accumulated_json_property_joins_chunks():
    """accumulated_json property must return the joined string for backward compat."""
    from litellm.llms.vertex_ai.gemini.vertex_and_google_ai_studio_gemini import (
        ModelResponseIterator,
    )
    ...
    iterator.accumulated_json_chunks = ["hello", " ", "world"]  # mutable-ok: direct test setup
    assert iterator.accumulated_json == "hello world"


def test_accumulated_json_chunks_cleared_after_successful_parse():
    """accumulated_json_chunks must be [] after a successful json.loads."""
    from litellm.llms.vertex_ai.gemini.vertex_and_google_ai_studio_gemini import (
        ModelResponseIterator,
    )
    ...
    valid_chunk = '{"candidates": [{"content": {"parts": [{"text": "hi"}], "role": "model"}}]}'
    iterator.handle_accumulated_json_chunk(chunk=valid_chunk)
    assert iterator.accumulated_json_chunks == []


def test_accumulated_json_list_buffer_performance():
    """1000-shard accumulation of 2 KB each must complete in under 100 ms (list is O(n), str+= was O(n^2))."""
    import time
    ...
    for _ in range(1000):
        iterator.handle_accumulated_json_chunk(chunk=shard)
    elapsed_ms = (time.perf_counter() - t0) * 1000
    assert elapsed_ms < 100, (
        f"1000-shard list-buffer accumulation took {elapsed_ms:.1f} ms (expected < 100 ms); "
        "O(n^2) string-copy regression?"
    )
